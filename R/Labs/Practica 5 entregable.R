...
# De esta manera mejoramos la visualización de los dato. Para los datos brutos se usa sin transformar, pero para visualizarlo es más útil
# El histograma es más visual
data2 = log2(data)
...
# Main, mediana (linea dentro de la caja)
# Cuartiles (los bordes de la caja; Q1 = 25%, Q3 = 75%)
# Rango intercuartilítico (IQR = Q3-Q1; la altura de las cajas)
# Bigotes (se extienden desde los cuartiles hasta 1.5 x IQR o hasta el valor más extremo dentro de ese rango)
# Outliers (los puntos que están fuera de los bigotes)
# Saca un boxplot para cada columna. Con eltítulo GSE5583-boxplots. Las = 2 hace que los nombres salgan en vertical
...
boxplot(data, col=c("blue", "blue", "blue",
	"orange", "orange", "orange"),
	main = "GSE5583 - boxplots", las=2)
# El boxplot sin transformar no queda bien	
...
# El clustering es para comprobar que hemos conseguido las muestras bien, con patrones simples
# Nos dice si hemos cogido muestras con genes similares, agrupa WT en un cluster y KO en otro

#######################################
# Análisis de Expresión Diferencial 
#######################################
# Hay un listado de genes con expresiones KO o WT y vamos a ver qué genes son lo suficientemente distintos como para ver si tienen efecto o un papel importante en su expresión

# Primero separamos las dos condiciones. ¿Qué tipo de datos has generado?
# Hemos generado una variable WT con 3 variables dentro y una variable KO con sus tres variables
head(data)
wt <- data[,1:3]
ko <- data[,4:6]
class(wt)
head(wt)
head(ko)
# De esta manera se calcula una matriz de datos
...
# Quiero poner el tope de los ejes en el valor más alto. La media más alta es 37460.5
...
# Si la correlación fuese perfecta estarían pegados a la línea roja
abline(0, 1, col = "red")
...
# OJO que aquí usamos los datos SIN TRANSFORMAR. ¿Por qué? porque para analizar los datos es mejor usar los raw data al ser más preciso
# ¿Cuántas valores tiene cada muestra?
#Primero he creado un vector de p values vacio, lo mismo de estadísticas. Con un bucle for para cada gen de la tabla crea un vector x que tenga los tres valores en WT y los tres KO
nrow(data) #número de genes = valores por muestra, 12488
ncol(data) #número de muestras, 6
pvalue = NULL 
tstat = NULL 
for(i in 1 : nrow(data)) { #Para cada gen
	...
	# Hacemos el test y guardamos en una variable
	t = t.test(x, y)
	
	# Añadimos el p-value a la lista (saca los p-values y los añadimos a la variable
	pvalue[i] = t$p.value
	# Añadimos las estadísticas a la lista
	tstat[i] = t$statistic
}
# Le muestras los primeros
head(pvalue)

# Ahora comprobamos que hemos hecho TODOS los cálculos
# Misma longitud que genes que tenga
length(pvalue)
# Al no haber evaluado la normalidad, no podemos saber si tiene una distribución normal
# Hacemos un histograma de los p-values.
# ¿Qué pasa si le ponemos con una transformación de -log10? Que transformamos los datos para visualizarlos mejor
# El primero son los p values brutos y el segundo son los p values transformados.
...
# Volcano plot es un gráfico que representa la diferencia dde media con los p-values transformados. Los puntos son los genes.
...
# Se hace un filtro de media y uno de p-value y el corte de p-value es 0.01
diff.mean_cutoff = 2
pvalue_cutoff = 0.01
abline(v = diff.mean_cutoff, col = "blue", lwd = 3)
#abline(v = -diff.mean_cutoff, col = "red", lwd = 3)
abline(h = -log10(pvalue_cutoff), col = "green", lwd = 3)
#Volcano plot para analizar expresiones de genes, representa en el eje x las diferencias de medias. En el eje de y los p values transformados. Los puntos son genes. Los genes que tengan mims expresion en wild type y ko, estarán cercanos al 0, wild type positivos y ko negativos. La linea del dos es que los genes que pasan el corte de significancia (es significativo a partir de o.o1 aquí representado con el 2) estarán por encima de esa línea

# Ahora buscamos los genes que satisfagan estos criterios
# Primero hacemos el filtro para la diferencia de medias (fold)
# Filter nos muestra cuantos genes están expresados
...
# Ahora vamos a marcar los que estarían sobreexpresados (rojo) y reprimidos (azul). ¿Por qué parece que están al revés? La sobreexpresión la asociamos con valores positivos por lógica, aquí es alrevés por la decisión del principio de hacer diferencia de medias. 
...
# ¿Qué es cada parámetro que hemos usado dentro de la función heatmap? Los genes diferencialmente expresados, para ver los bloques de expresión
...
# Los rojo es sobreexpersión y lo azul reprimido
